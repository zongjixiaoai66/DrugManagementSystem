    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yaopinOrder !=null){
            ...
            if(yaopin != null){
            BeanUtils.copyProperties( yaopin , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
...
//        else if("用户".equals(role))
//            yaopinOrder.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(yaopinOrder.getYaopinOrderFile()) || "null".equals(yaopinOrder.getYaopinOrderFile())){
                yaopinOrder.setYaopinOrderFile(null);
        }
        if("".equals(yaopinOrder.getYaopinOrderYesnoText()) || "null".equals(yaopinOrder.getYaopinOrderYesnoText())){
                yaopinOrder.setYaopinOrderYesnoText(null);
        }
    ...
    */
    ...
    public R shenhe(@RequestBody YaopinOrderEntity yaopinOrderEntity, HttpServletRequest request){
        logger.debug("shenhe方法:,,Controller:{},,yaopinOrderEntity:{}",this.getClass().getName(),yaopinOrderEntity.toString());

        YaopinOrderEntity oldYaopinOrder = yaopinOrderService.selectById(yaopinOrderEntity.getId());//查询原先数据
        String role = String.valueOf(request.getSession().getAttribute("role"));
        YuangongEntity yuangong=null;
        if("员工".equals(role)){
            yuangong = yuangongService.selectById((Integer) request.getSession().getAttribute("userId"));
        }

        if(yaopinOrderEntity.getYaopinOrderYesnoTypes() == 2){//通过
            if(yuangong != null)
                yaopinOrderEntity.setYaopinOrderYesnoText(yaopinOrderEntity.getYaopinOrderYesnoText()+"审核人:"+yuangong.getYuangongName()+";审核人编号:"+yuangong.getYuangongUuidNumber());
        }else if(yaopinOrderEntity.getYaopinOrderYesnoTypes() == 3){//拒绝
        }
        yaopinOrderEntity.setYaopinOrderShenheTime(new Date());//审核时间
        yaopinOrderService.updateById(yaopinOrderEntity);//审核
        return R.ok();
    }
...
//                            yaopinOrderEntity.setYaopinOrderFile(data.get(0));                    //医生证明 要改的
...
//                            yaopinOrderEntity.setYaopinOrderYesnoTypes(Integer.valueOf(data.get(0)));   //审核状态 要改的
//                            yaopinOrderEntity.setYaopinOrderYesnoText(data.get(0));                    //审核意见 要改的
//                            yaopinOrderEntity.setYaopinOrderShenheTime(sdf.parse(data.get(0)));          //审核时间 要改的
        ...
        YaopinOrderEntity yaopinOrder = yaopinOrderService.selectById(id);
            if(yaopinOrder !=null){
                ...
                //级联表
                    YaopinEntity yaopin = yaopinService.selectById(yaopinOrder.getYaopinId());
                if(yaopin != null){
                    BeanUtils.copyProperties( yaopin , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYaopinId(yaopin.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(yaopinOrder.getYonghuId());
    ...
    */
    ...
    public R add(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        Integer addressId = Integer.valueOf(String.valueOf(params.get("addressId")));
            ...
            String yaopinOrderFile = String.valueOf(params.get("yaopinOrderFile"));//医生证明
//            Integer yaopinOrderYesnoTypes = Integer.valueOf(String.valueOf(params.get("yaopinOrderYesnoTypes")));//审核状态
//            String yaopinOrderYesnoText = String.valueOf(params.get("yaopinOrderYesnoText"));//审核意见
        ...
        for (Map<String, Object> map : yaopins) {
           //取值
            ...
            yaopinOrderEntity.setYaopinOrderFile(yaopinOrderFile);//医生证明 ？？？？？？
            yaopinOrderEntity.setYaopinOrderTypes(101);//订单类型
            if(yaopinEntity.getChufangyaoTypes()==1){
                yaopinOrderEntity.setYaopinOrderYesnoTypes(1);//审核状态
            }else {
                yaopinOrderEntity.setYaopinOrderYesnoTypes(null);//审核状态

            }
            yaopinOrderEntity.setYaopinOrderYesnoText(null);//审核意见 ？？？？？？
            yaopinOrderEntity.setYaopinOrderShenheTime(null);//审核时间 ？？？？？？
            yaopinOrderEntity.setYaopinOrderPaymentTypes(yaopinOrderPaymentTypes);//支付类型
            yaopinOrderEntity.setInsertTime(new Date());//购买时间
    ...
    */
    ...
    public R refund(Integer id, HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
            ...
            yaopinOrder.setYaopinOrderYesnoTypes(null);
